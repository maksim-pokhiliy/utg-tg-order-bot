...
describe("the dedupe predicate", () => {
  ...
  it("sends an edited order that reuses the key of a delivered one", () => {
    const edited = createAttempt(
      buildEnvelope({ total: "300.00", idempotency_key: PINNED_KEY })
    );
  ...
  it("never suppresses a keyless order, even when a prior is offered", () => {
    const attempt = createAttempt(
      buildEnvelope({ idempotency_key: undefined })
    );

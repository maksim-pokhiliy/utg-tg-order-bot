...
describe("the store can never gate the send", () => {
  ...
  it("still relays the order and answers 200 when the dedupe verdict throws", async () => {
    const stub = stubRelayFetch();
    ...
    vi.doMock("../src/store.js", async () => {
      ...
      return {
        ...actual,
        readDedupeVerdict: (): never => {
          throw new RangeError("simulated verdict defect");
        },
      };
    });
    ...
    try {
      ...
      await expect(response.text()).resolves.toBe(FROZEN_SUCCESS);
      expect(readTelegramCalls(stub)).toHaveLength(1);
      expect(joinAllLogged(logs)).toContain("order_store_unavailable");
      expect(joinAllLogged(logs)).not.toContain("simulated verdict defect");
    } finally {
      vi.doUnmock("../src/store.js");
      vi.resetModules();
    }

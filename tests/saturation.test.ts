...
describe("the composed message can never exceed the telegram limit", () => {
  ...
  it("leaves the cart room to breathe once the header is saturated", () => {
    for (const delivery of Object.values(SATURATED_DELIVERIES)) {
      const header = renderOrder({
        kind: "v2",
        payload: { ...saturatedPayloadV2(delivery), cart: [] },
      });

      expect(header.length).toBeLessThan(TELEGRAM_LIMIT);
    }
  });
});

...
The message keeps the field order, emoji and bold labels the operators already know, rendered in HTML parse mode with every interpolated value escaped. Those labels are Ukrainian whatever the order's `locale`: the shop is Ukrainian and the operators' chat is internal, so there is nobody in it an English gloss would help. `locale` still chooses the number style for the money figure — it does not choose the language of the message.
